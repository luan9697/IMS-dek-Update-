...
public class ClassRegister implements SipListener {
	...
	public ClassRegister(NewController gui) {
		try {
			this.GUI = gui;
...
//			sIP = InetAddress.getLocalHost().getHostAddress();
			...
			//iPort = GUI.getPort();
			iPort = 6060; // port su dung trao doi SIP
			String protocol = "udp"; // giao thuc trao doi la UDP
	...
	public void sendRequest(String typeRequest) {// gui yeu cau tu UAC
		try {

			String username = this.GUI.getUsername();
			switch (typeRequest) {
			case Request.REGISTER: {
				...
				nameAddress = "sip:"+ username +"@open-ims.test";
				...
				// tao SIP request va gan cac thong tin vao.
				...
				System.out.println("Gởi : " + request.toString());
				break;
			}
			...
			case Request.INVITE: {
				...
				Address toAddress = addressFactory.createAddress("sip:"+ username +"@open-ims.test");
				// địa chỉ AOR
				Address fromAddress = addressFactory.createAddress("sip:"+ username +"@open-ims.test");
				// tạo To header
				ToHeader toHeader = headerFactory.createToHeader(toAddress, null);
				// tạo From Header
				...
				System.out.println(clientTransaction.getState().toString());
				System.out.println("Gởi : " + request.toString());// hiển thị nội dung Request
				break;
			}
			...
			case Request.BYE: {
				...
				System.out.println(BYEClientTransaction.getState().toString());
				System.out.println("Gởi : " + ByeRequest.toString());
	...
	public void sendResponse(String typeRequest, Response response) {// gui lai phan hoi tu UAS
		try {
			...
			case Request.REGISTER: {
				...
				System.out.println(clientTransaction.getState().toString());
				System.out.println(" Gởi : " + request.toString());// hiển thị nội dung Request trong txtHIENTHI
				break;
			}
	...
	public void processRequest(RequestEvent requestEvent) {
		try {
			...
			GUI.setTextArea("Nhận : " + request.toString());
			...
			if (request.getMethod().equals("REGISTER")) {
				...
				System.out.println("Gởi : " + response.toString());
			...
			// khi request nhận được là INVITE
				...
				System.out.println(BYEServerTransaction.getState().toString());
				GUI.setTextArea("Gởi : " + response.toString());
			}
		} catch (Exception ex) {
			System.out.println(ex.getMessage());
		}
...
//	@SuppressWarnings("deprecation")
	...
	public void processResponse(ResponseEvent responseEvent) {
		...
		try {
			...
			GUI.setTextArea("Nhận : " + response.toString());
			...
			if (cSeqHeader.getMethod().equals("REGISTER")) {
				...
				// kiểm tra StatusCode của response có phải là 200
					...
					GUI.setTextArea("Gởi : " + ackRequest.toString());
					info =  "REGISTET-200OK";
				}
